...
Backed by AWS Secrets Manager; cached in-process (5-min TTL, see
``core.cache.TTLCache``) using the same idiom ``core.settings`` already uses
for org config, rather than the sync-only ``aws-secretsmanager-caching``
client the original Omni-Channel plan called for (⚠ ADAPTED — see
app/services/omnichannel/CLAUDE.md §6.2). The cache used to be Redis-backed;
it moved in-process when the platform collapsed to a single box/process
(docs/architecture/single-box-mvp.md) — same TTL, same invalidate-on-write
semantics, no more network hop.
...
async def get_secret(org_id: str, service_type: str, key: str) -> dict[str, Any]:
    ...
    _cache.set(cache_key, raw, ttl_seconds=_CACHE_TTL_SECONDS)
    log.info(
        "secret.cache_miss",
        extra={"org_id": org_id, "service_type": service_type, "key": key},
    )
...
async def put_secret(org_id: str, service_type: str, key: str, value: dict[str, Any]) -> None:
    ...
    _cache.delete(_cache_key(org_id, service_type, key))
    log.info(
        "secret.write",
        extra={"org_id": org_id, "service_type": service_type, "key": key},
    )
