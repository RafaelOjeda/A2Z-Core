"""Settings — org-level configuration (Design §2.6).

Reads go straight to DynamoDB, applying defaults for any missing field --
well inside the documented < 50ms target without a cache in front of it, now
that the platform runs as a single box/process
(docs/architecture/single-box-mvp.md). (A prior revision cached reads in
Redis with a 5-min TTL; removed as a pure deletion when Redis was retired --
see git history if that idiom is ever needed again at higher read volume.)
Writes update DynamoDB and audit ``settings.changed``. The invoice counter is
an atomic DynamoDB ``ADD`` so it never collides or skips.
...
async def get_org_settings(org_id: str) -> OrgSettings:
    ...
    One DynamoDB read. Performance: < 50ms.
    """
    ...
    return OrgSettings(org_id=org_id, **merged)
...
async def set_org_settings(org_id: str, changes: dict[str, Any], changed_by: str) -> OrgSettings:
    ...
    Validates field names, writes DynamoDB, and logs ``settings.changed``.
    Raises SettingsError on an unknown field. Performance: < 100ms.
    """
