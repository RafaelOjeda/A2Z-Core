...
> **Single linear head (resolved 2026-07-20).** `app/services/omnichannel/migrations/versions/`
> previously contained a second, orphaned root migration
> (`0001_initial_schema.py`, an earlier hand-written draft whose index names
> like `ix_channel_connections_org_id` never matched the autogenerated
> convention the rest of the schema uses). It shared `down_revision = None`
> with the real baseline, so Alembic saw **two heads** and
> `alembic upgrade head` was ambiguous. The orphan has been deleted; the
> chain above is now the single head, and `alembic upgrade head` against a
> fresh database was verified end to end (`<base>` → `1bfacee578a4` →
> `0002_inbox_index` → `0003_message_dedup_key`, 10 tables). Note the test
> suite still builds the schema via `Base.metadata.create_all()`
> (`tests/integration/omnichannel/conftest.py`), not by running migrations —
> so the migration chain's own correctness is verified out of band, not by
> the suite. Authoritative chain: `0001_baseline_schema.py` →
